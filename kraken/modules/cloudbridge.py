...
class CloudBridgeModule(BaseModule):
    ...
    def run(self, ctx: EngagementContext, **kwargs: object) -> list[AttackResult]:
        ...
        if not HAS_REQUESTS:
            return [AttackResult("cloud-bridge", "setup", "FAILED", notes="Install requests: pip install requests")]
    ...
    def _probe_imds(self, ctx: EngagementContext, cloud_name: str, endpoint: dict[str, Any]) -> AttackResult | None:
        url = endpoint["url"]
