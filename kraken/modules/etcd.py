...
class EtcdModule(BaseModule):
    ...
    def _etcd_v2_dump(self, ctx: EngagementContext, host: str) -> List[AttackResult]:
        ...
        return [AttackResult("etcd", "v2_probe", "INFO",
                             notes="etcd v2 not accessible or requires auth")]
    ...
    def _etcd_v3_check(self, ctx: EngagementContext,
        ...
        etcdctl = ("etcdctl" if not cert_dir else
                   f"etcdctl --cacert={cert_dir}/ca.crt --cert={cert_dir}/server.crt --key={cert_dir}/server.key")
        try:
            proc = subprocess.run(
