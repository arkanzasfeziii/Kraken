...
class EscapeModule(BaseModule):
    ...
    def _check_technique(self, tech: dict[str, Any]) -> AttackResult:
        name = tech["name"]
        try:
            proc = subprocess.run(tech["check"], shell=True, capture_output=True, text=True, timeout=5)
            ...
            if tech["indicator"] in output:
                ...
                return AttackResult("escape", name, "INFO", notes=f"{name}: not applicable")
    ...
    def _check_capabilities(self) -> list[AttackResult]:
        ...
        try:
            proc = subprocess.run(
                "cat /proc/self/status | grep Cap", shell=True, capture_output=True, text=True, timeout=3
            )
