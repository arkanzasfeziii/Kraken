...
from kraken.utils.helpers import b64d, scan_for_secrets, k8s_connect
...
class SecretDumpModule(BaseModule):
    ...
    def _scan_configmaps(self, ctx: EngagementContext,
                          namespaces: List[str]) -> List[Dict[str, Any]]:
                ...
                for cm in cms.items:
                    for k, v in (cm.data or {}).items():
                        ...
                        if found:
                            ...
                            log(f"[SecretDump] ConfigMap secret: {ns}/{cm.metadata.name}.{k}: {found[0]['type']}", "CRIT")
            except Exception:
                pass
        if hits:
